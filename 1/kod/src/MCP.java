...
public class MCP {
    ...
    public void startlearnig() throws FileNotFoundException {
        PrintWriter zapis = new PrintWriter("MPC_czas.txt");
        PrintWriter zapis1 = new PrintWriter("MPC_bledy_wal.txt");
        PrintWriter zapis2 = new PrintWriter("MPC_MSE_wal.txt");
        PrintWriter zapis12 = new PrintWriter("MPC_bledy.txt");
        PrintWriter zapis22 = new PrintWriter("MPC_MSE.txt");
        PrintWriter zapis3 = new PrintWriter("MCP logi.txt");
        ZbierzDane();
    ...
    private void check() {
        ...
        for (int j = 0; j < 30; j++) {
            ...
            if(Math.abs(wynikf-walidujace[j][4])<0.1)
                    poprawny++;
            else
                {
                    error++;
        ...
        for (int j = 0; j < 70; j++) {
            ...
            double wynikf = f(sumuj); //sprawdzanie z wynikiem w pliku
            if(Math.abs(wynikf-uczace[j][4])<0.1)
                poprawny++;
            else
            {
                error1++;
    ...
    {
        return(1/(1+Math.pow(Math.E,(-0.1*sumuj))));
    }



    private void uczMCP(double[] wartosciNowe) {
        ...
        if(Math.abs(wynikf-wartosciNowe[4])>0.1)
        {

            for(int i=0;i<4;i++)
            {
                wagi[i]=wagi[i]+0.01*((wartosciNowe[4]-wynikf)*wartosciNowe[i]);
            }
        }
